...
def server(input, output, session):
    ...
    @render.plot()
    def evscatter():
        esz = 20.*(edf[input.eCol()] - min(edf[input.eCol()]))/(max(edf[input.eCol()]) - min(edf[input.eCol()]))
        vsz = 20.*(vdf[input.vCol()] - min(vdf[input.vCol()]))/(max(vdf[input.vCol()]) - min(vdf[input.vCol()]))
        fig, ax = plt.subplots()
